...
### 新增
- **Code Context Basket 工作流增强** —— 新增 `🧺 Clear And Add` 命令，可在添加新的代码片段前快速清空当前篮子；篮子侧边栏支持“一键复制为 Markdown”，方便分享整理好的代码上下文。
- **Code Context Basket 历史时间线** —— 自动记录每次篮子快照，并通过命令与侧边栏历史面板提供打开、编辑、删除等操作。

### 变更
- **命令菜单识别度** —— 与路径相关的命令统一使用路标 emoji（`🪧`），备份相关命令使用档案夹 emoji（`🗂️`），使右键菜单更易扫读。
- **右键菜单分组** —— 将 `Copy Code Context` 与“行内容弹窗编辑”从 `🧺 Code Context Basket` 操作中拆分出来，便于快速定位。
- **历史删除行为** —— 移除软删除；删除历史记录会直接永久移除。
- **篮子面板精简** —— 隐藏 `Load`、`刷新`、`更多操作` 按钮，让界面更聚焦核心流程。
- **工具栏样式统一** —— 预览面板与 Code Context Basket 的顶部按钮统一采用新的 `cp-toolbar`/`cp-btn` 风格，带来一致的界面体验。

## [0.2.7] - 2025-11-23

### 新增
- **灯泡菜单中的复制代码上下文** - 现有的 `codepath.copyCodeContext` 命令现在也可以通过编辑器灯泡（Quick Fix）菜单直接访问，并与行内容弹窗编辑入口一起展示，方便一键复制带文件路径和行号范围的可分享代码片段。
...
### 修复
- 修复了 Vitest 中 VSCode 模块的 mock 配置问题，解决测试失败
- 通过统一的测试设置提高了测试稳定性和可靠性
- 测试通过率达到 99.1%（1265/1276 个测试通过）
- 记录了边缘情况和需要功能决策的跳过测试

### 技术改进
- 添加统一的测试设置文件 `src/__mocks__/testSetup.ts`
- 添加 fs 和 path 模块的 mock 文件
- 修复 vscode.ts mock 的导出格式兼容性
- 移除测试文件中重复的 vi.mock 声明

## [0.2.2] - 2025-10-21

### 新增
- `Code Path → Copy Code Context` 命令支持将选中代码的相对路径、行号范围与代码片段一次性复制到剪贴板，适用于多段选区与跨文件分享场景。

### 变更
- 右键菜单在编辑器存在选区时才显示 `Copy Code Context`，避免空选区触发误操作。
