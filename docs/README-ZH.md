...
Code Path Marker 是一款 VS Code 扩展，支持在阅读或调试代码时，以交互式节点的方式记录执行路径。它兼顾文本树与图形化视图，让开发者可以在复杂逻辑中快速定位、追踪和分享关键代码片段。

## 核心功能地图

| 能力模块 | 亮点功能 |
| --- | --- |
| 路径标注 | 统一“Code Path”右键菜单、根/父/子/兄弟节点、文件/文件夹标注、节点复制/粘贴/剪切/排序 |
| 智能追踪 | 多策略位置匹配（精确/邻近/全文/模糊）、SHA-256 代码指纹、位置置信度评估、自动更新节点位置与片段 |
| 图谱可视化 | 文本树与 Mermaid 图表双视图、实时刷新、分屏布局、预览面板自定义菜单、状态栏节点提示 |
...
- **文档与教学**：输出 Mermaid 图表，帮助新成员或学员理解业务流程。
...
3. 在左侧 Code Path 预览面板中点击节点即可跳转至对应代码，并查看实时树状结构或 Mermaid 图。
...
| 切换视图格式 | `Ctrl+Shift+T` | 文本与 Mermaid 图互换 |
...
- 文本视图提供极简树形结构，Mermaid 视图用于展示流程图；两种视图支持无缝切换。
...
2. **Mermaid 图渲染失败怎么办？** 扩展内置校验器会在渲染失败时给出错误提示，可检查节点命名或在设置中默认使用文本视图。
