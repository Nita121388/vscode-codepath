...
- **文件与文件夹备份管理** - 从文件浏览器（支持多选批量）或状态栏菜单快速备份文件/文件夹至 `.codepath/file-backups`，自动保留最新备份，一键恢复或清空所有备份
- **跨平台** - 全平台兼容
...
- **File & Folder Backup Management** - Quickly back up selected files/folders (explorer multi-select supported) from explorer or status bar menu to `.codepath/file-backups`, keep only the latest backup per resource, restore with one click, or clear all backups safely
